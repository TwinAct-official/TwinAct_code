...
from transformers import AutoTokenizer, PretrainedConfig, CLIPModel
import wandb
...
local_diffuser_path = './diffusers/src'
...
from diffusers.models.attention_processor import LoRAAttnProcessor, LoRAAttnProcessor2_0, AttnProcessor, AttnProcessor2_0, XFormersAttnProcessor, LoRAXFormersAttnProcessor
...
from transformers import CLIPTextModel, CLIPTextModelWithProjection, CLIPVisionModel
...
def coustom_import_model_class_from_model_name_or_path(
        pretrained_model_name_or_path: str, revision: str, subfolder: str = "text_encoder"
):
    text_encoder_config = PretrainedConfig.from_pretrained(
        pretrained_model_name_or_path, subfolder=subfolder, revision=revision
    )
    print("load from:", pretrained_model_name_or_path)
    print(text_encoder_config)
    model_class = text_encoder_config.architectures[0]

    if model_class == "CustomCLIPTextModel":
        return CustomCLIPTextModel
    elif model_class == "CustomCLIPTextModelWithProjection":
        return CustomCLIPTextModelWithProjection
    else:
        raise ValueError(f"{model_class} is not supported.")
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--train_stage", 
        choices=["do_ti", "load_ti_do_lora", "load_ti_do_both", "do_both", "load_ti_do_both_multi"], 
        required=True)
    ...
    # inserting anybody hyper param
    parser.add_argument("--act_base_dir", type=str, default="./base_3_512.pt")
    parser.add_argument("--mlp_hidden", type=int, default=640)
    ...
    if args.train_stage in {'load_ti_do_lora', 'load_ti_do_both'}:
    ...
    print("=====training tokens=======", len(args.placeholder_tokens), len(args.initializer_tokens))
...
class MyControlNet(ControlNetModel):
    def get_embedding(self, sample):
        controlnet_cond = self.controlnet_cond_embedding(sample)
        return controlnet_cond


class Mlp(nn.Module):
    def __init__(self, input_size=320, hidden_size=640, output_size=512):
...
class CustomCLIPTextEmbeddings(nn.Module):
    def __init__(self, config: CLIPTextConfig):
        super().__init__()
        embed_dim = config.hidden_size

        self.token_embedding = nn.Embedding(config.vocab_size, embed_dim, padding_idx=config.pad_token_id)

        self.position_embedding = nn.Embedding(config.max_position_embeddings, embed_dim)

        self.register_buffer("position_ids", torch.arange(config.max_position_embeddings).expand((1, -1)))

        act_base_dir = "./base_3_512.pt"
        self.action_base = torch.load(act_base_dir)
        pose_dir = "./action_features.pt"
        self.pose_features = torch.load(pose_dir)

        self.num_words, self.mlp_out = self.action_base.shape[0], self.action_base.shape[1] - 1
        self.mlp_list = nn.ModuleList(
            [Mlp(input_size=320, hidden_size=640, output_size=self.mlp_out) for _ in range(self.num_words)])
    ...
    def forward(self,
        ...
        if inputs_embeds is None:
            ...
            # print(input_ids) #(bs,77)
            # print(inputs_embeds.shape) # torch.Size([bs, 77, 768])
            for batch_index in range(input_ids.shape[0]):  # 遍历batch中的每个样本
                for seq_index, iid in enumerate(input_ids[batch_index]):  # 遍历序列中的每个元素
                    if isinstance(iid, torch.Tensor) and iid.numel() == 1:  # numel() 返回张量中元素的数量
                        iid = iid.item()  # 将张量转换为整数
                    if iid >= 49408:
                        action_name = {}
                        # pose feature (1, 320)
                        pose_feature = self.pose_features[action_name[iid]].to(inputs_embeds.device)  # (1,320)
                        mlp_num = (iid - 49408) % self.num_words # 判断是<heart1/2/3>
                        a_i = self.mlp_list[mlp_num](pose_feature)  # (1, 512)
                        v = self.action_base[mlp_num][0].to(inputs_embeds.device)  # (2048)
                        b_i = self.action_base[mlp_num][1:].to(inputs_embeds.device) # (512, 2048)

                        if inputs_embeds.shape[-1] == 768:
                            embeddings[batch_index, seq_index] += torch.matmul(a_i, b_i)[0][:768]
                        else:
                            embeddings[batch_index, seq_index] += torch.matmul(a_i, b_i)[0][768:]
        else:
...
class CustomCLIPTextModel(CLIPTextModel):
    ...
    def __init__(self, config: CLIPTextConfig):
        ...
        print("====CustomCLIPTextModel=====")
        self.text_model.embeddings = CustomCLIPTextEmbeddings(config)
        print(self.text_model.embeddings)

    def get_extra_embeddings(self) -> nn.Module:
        return self.text_model.embeddings.extra_embedding
...
class CustomCLIPTextModelWithProjection(CLIPTextModelWithProjection):
    ...
    def __init__(self, config: CLIPTextConfig):
        ...
        print("====CustomCLIPTextModelWithProjection=====")
        self.text_model.embeddings = CustomCLIPTextEmbeddings(config)
        print(self.text_model.embeddings)
...
def main(args):
    ...
    text_encoder_one_config = text_encoder_one_.config
    text_encoder_one = CustomCLIPTextModel(text_encoder_one_config)

    text_encoder_one.text_model.encoder.load_state_dict(
        text_encoder_one_.text_model.encoder.state_dict(), strict=False)
    text_encoder_one.text_model.embeddings.token_embedding.load_state_dict(
        text_encoder_one_.text_model.embeddings.token_embedding.state_dict(), strict=False)
    text_encoder_one.text_model.embeddings.position_embedding.load_state_dict(
        text_encoder_one_.text_model.embeddings.position_embedding.state_dict(), strict=False)

    text_encoder_two_config = text_encoder_two_.config
    text_encoder_two = CustomCLIPTextModelWithProjection(text_encoder_two_config)

    text_encoder_two.text_model.encoder.load_state_dict(
        text_encoder_two_.text_model.encoder.state_dict(), strict=False)
    text_encoder_two.text_model.embeddings.token_embedding.load_state_dict(
        text_encoder_two_.text_model.embeddings.token_embedding.state_dict(), strict=False)
    text_encoder_two.text_model.embeddings.position_embedding.load_state_dict(
        text_encoder_two_.text_model.embeddings.position_embedding.state_dict(), strict=False)
    ...
    # mlp_list = [Mlp(input_size=320, hidden_size=args.mlp_hidden, output_size=mlp_out) for _ in range(num_words)]

    # preprocessing, need to be frozen
    openpose = OpenposeDetector.from_pretrained("./ControlNet")
    clipvision = CLIPVisionModel.from_pretrained("./clip-vit-large-patch14")
    controlnet = MyControlNet.from_pretrained('./control_v11p_sd15_openpose')
    ...
    if args.train_stage in {'load_ti_do_lora', 'load_ti_do_both'}:
        ...
        text_encoder_cls_one = coustom_import_model_class_from_model_name_or_path(
            args.pretrained_ti_path, subfolder="text_encoder", revision=args.revision
        )
        text_encoder_cls_two = coustom_import_model_class_from_model_name_or_path(
        ...
        with safe_open(args.pretrained_ti_path+"/text_encoder/model.safetensors", framework="pt", device=0) as f:
            for name in f.keys():
                if 'mlp_list' in name:
                    parts = name.split('.')
                    mlp_num = int(parts[-3])
                    ...
                    new_param = torch.nn.Parameter(param_tensor)
                    
                    if layer_type == 'fc':
                        if param_type == 'weight':
                            text_encoder_one.text_model.embeddings.mlp_list[mlp_num].fc.weight = new_param
                        elif param_type == 'bias':
                            text_encoder_one.text_model.embeddings.mlp_list[mlp_num].fc.bias = new_param
                    elif layer_type == 'output':
                        if param_type == 'weight':
                            text_encoder_one.text_model.embeddings.mlp_list[mlp_num].output.weight = new_param
                        elif param_type == 'bias':
                            text_encoder_one.text_model.embeddings.mlp_list[mlp_num].output.bias = new_param
        print("loaded mlp for text_encoder")
        
        with safe_open(args.pretrained_ti_path+"/text_encoder_2/model.safetensors", framework="pt", device=0) as f:
            for name in f.keys():
                if 'mlp_list' in name:
                    parts = name.split('.')
                    mlp_num = int(parts[-3])
                    ...
                    if layer_type == 'fc':
                        if param_type == 'weight':
                            text_encoder_two.text_model.embeddings.mlp_list[mlp_num].fc.weight = new_param
                        elif param_type == 'bias':
                            text_encoder_two.text_model.embeddings.mlp_list[mlp_num].fc.bias = new_param
                    elif layer_type == 'output':
                        if param_type == 'weight':
                            text_encoder_two.text_model.embeddings.mlp_list[mlp_num].output.weight = new_param
                        elif param_type == 'bias':
                            text_encoder_two.text_model.embeddings.mlp_list[mlp_num].output.bias = new_param
        print("loaded mlp for text_encoder_2")
        ...
        with torch.no_grad():
            for pi, ii in zip(placeholder_token_ids_one, token_ids_one):
                token_embeds_one[pi] = token_embeds_one[ii].clone() # torch.float32
    ...
    clipvision.requires_grad_(False)

    if args.train_stage == "load_ti_do_lora":
        ...
        text_encoder_one.text_model.embeddings.token_embedding.requires_grad_(False)
        ...
        text_encoder_two.text_model.embeddings.token_embedding.requires_grad_(False)
    ...
    clipvision.to(accelerator.device, dtype=weight_dtype)
    controlnet.eval()
    clipvision.eval()
    ...
    text_encoder_one.text_model.embeddings.action_base.to(accelerator.device)
    text_encoder_two.text_model.embeddings.action_base.to(accelerator.device)
    ...
    if args.train_stage == "load_ti_do_both" or args.train_stage == "load_ti_do_lora":
    ...
    for name, model in (
        ...
        for n, p in model.named_parameters():
            if "text_projection" in n:
                if args.train_stage in {'do_ti', 'load_ti_do_lora'}:
                    continue
                elif args.train_stage in {'load_ti_do_both', 'do_both'}:
                    continue  # TODO: 
            if p.requires_grad:
                params.append(p)
    ...
    def get_pose_embedding(image):
        if isinstance(image, torch.Tensor):
            image = tensor_to_pil(image)
        pose_img = openpose(image)
        pose_img_tensor = trans(pose_img)
        pose_img_tensor = pose_img_tensor.unsqueeze(0).to(accelerator.device, dtype=weight_dtype)
        embedding = controlnet.get_embedding(pose_img_tensor)
        pool = torch.nn.AdaptiveAvgPool2d((1,1))
        pool_embedding = pool(embedding)
        pool_embedding = torch.squeeze(pool_embedding)
    ...
    elif args.train_stage == 'load_ti_do_lora' and not args.train_text_encoder:
    ...
    start_sim_epoch = int(args.num_train_epochs * 0.4)
    start_sim_step = int(noise_scheduler.config.num_train_timesteps * 0.8)
    clip_resize = transforms.Resize([224, 224])
    simloss = CosineSimilarityLoss()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder_one), \
                accelerator.accumulate(text_encoder_two):
                ...
                sloss = None
                # add sim loss
                if epoch > start_sim_epoch:
                    step_ind = torch.where(timesteps > start_sim_step)[0]
                    num_img = step_ind.shape[0]

                    if num_img > 0:
                        latents = model_pred[step_ind].half()
                        images = vae.decode(latents / vae.config.scaling_factor, return_dict=False)[0]
                        gt_imgs = pixel_values[step_ind]
                        images = clip_resize(images)
                        gt_imgs = clip_resize(gt_imgs)
                        image_emd = clipvision(images).pooler_output
                        gt_emb = clipvision(gt_imgs).pooler_output
                        sloss = simloss(image_emd.float(), gt_emb.float())
                ...
                if sloss is not None:
                    print('similarity loss!', sloss.item())
                    loss += 0.15 * sloss
                ...
                if args.train_stage in {'do_ti', 'do_both', 'load_ti_do_both'}:
